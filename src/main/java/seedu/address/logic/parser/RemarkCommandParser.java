...
public class RemarkCommandParser implements Parser<RemarkCommand> {
    ...
    public RemarkCommand parse(String args) throws ParseException {
        /* requireNonNull(args); [RemarkCommand should accept null values for removing remark.]*/
        ...
        if (argMultimap.getValue(PREFIX_REMARK).isPresent()) {
            remarkPersonDescriptor.setRemark(ParserUtil.parseRemark(argMultimap.getValue(PREFIX_REMARK).get()));
        }

        if (!remarkPersonDescriptor.isRemarkEdited()) {
            throw new ParseException(RemarkCommand.MESSAGE_REMARK_NOT_EDITED);
        }
